...
void GameLogic::update(void)
{
	...
	Bool generateForMP = (isMPGameOrReplay && m_frame > 0 && (m_frame % TheGameInfo->getCRCInterval()) == 0);
#ifdef DEBUG_CRC
...
#else
	Bool generateForSolo = isSoloGameOrReplay && m_frame > 0 && ((m_frame % REPLAY_CRC_INTERVAL) == 0);
#endif // DEBUG_CRC
